...
**Performance and modernization overhaul targeting PHP 8.5+**

**BREAKING CHANGES:**

- **All `define()` response code constants have been removed.** The 35+ global `NET_NNTP_PROTOCOL_RESPONSECODE_*` constants (e.g. `NET_NNTP_PROTOCOL_RESPONSECODE_OVERVIEW_FOLLOWS`) no longer exist. They have been replaced by a backed `enum ResponseCode: int` in `Responsecode.php`. Any code that references the old constants — including subclasses and external consumers — **will fatally error** until updated. Migration: replace `NET_NNTP_PROTOCOL_RESPONSECODE_OVERVIEW_FOLLOWS` with `ResponseCode::OverviewFollows->value` (or match on the enum case directly).
- Removed deprecated v1.0/v1.1 API compatibility shims from `getArticle()`, `getHeader()`, `getBody()`, `post()` and `getOverview()`. Callers using the legacy multi-parameter signatures must update to the current API.

**Performance improvements:**

- Moved `_clearOpensslErrors()` out of the per-line read loop in `_getTextResponse()` — previously called twice per line, now called once before/after the loop. Major improvement for SSL/TLS connections reading large responses.
- Replaced `preg_replace()` with `str_replace()` for dot-stuffing in `_sendArticle()` — avoids regex compilation overhead on every post/transfer.
- Replaced `preg_match()` with `strpos()`/`substr()` in `cmdListNewsgroups()` and `cmdXGTitle()` — significant for servers with 100k+ groups.
- Removed redundant `trim()` calls from all `cmd*()` methods — lines are already stripped of CRLF by `_getTextResponse()`.
- Eliminated pointless array copy in `cmdNewnews()` — previously copied `_getTextResponse()` element-by-element into an identical new array.
- Merged separate `fwrite()` calls in `_sendArticle()` into single writes to reduce TCP packet overhead.
- Cached logger debug flag in `$_debug` property — replaces ~20 scattered `$this->_logger->_isMasked(PEAR_LOG_DEBUG)` calls with a simple boolean check.
- Removed `func_num_args()`/`func_get_arg()` overhead from `getOverview()` hot path — v1.0 API detection no longer runs on every call.
- Simplified command string building across `cmdArticle()`, `cmdHead()`, `cmdBody()`, `cmdStat()`, `cmdOver()`, `cmdXOver()`, `cmdListActive()`, `cmdListNewsgroups()`, `cmdXHdr()`, `cmdXROver()`.

**Modernization (PHP 8.0+ / 8.5+):**

- `substr()` checks replaced with `str_ends_with()` and `str_starts_with()` throughout, including the `_getTextResponse()` hot loop and all range/message-id detection logic.
- Response codes converted from `define()` constants to a backed `enum ResponseCode: int` — resolved at compile time with full type safety.
- `require_once` changed to `require` — removes redundant file-existence checking overhead.
- Socket nulled on disconnect for faster `_isConnected()` short-circuit.
- `cmdStartTLS()` now correctly sets `$_encryption` after successful negotiation.
